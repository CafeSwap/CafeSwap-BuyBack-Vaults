...
// File contracts/interfaces/IBrewVault.sol

interface IBrewVault {
    ...
    function userInfo(address _user) external view returns (
        ...
        uint256 brewAtLastUserAction,
        uint256 lastUserActionTime
    );
}


// File contracts/CafeSingleVaults.sol

contract CafeSingleVaults is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 stake;
        // How many staked $BREW user had at his last action
        uint256 autoBrewShares;
        // Brew shares not entitled to the user
        uint256 rewardDebt;
    ...
    IERC20 constant public BREW = IERC20(0x790Be81C3cA0e53974bE2688cDb954732C9862e1);
    IBrewVault immutable public AUTO_BREW;
    IERC20 immutable public STAKED_TOKEN;

    // Runtime data
    mapping(address => UserInfo) public userInfo; // Info of users
    uint256 public accSharesPerStakedToken; // Accumulated AUTO_BREW shares per staked token, times 1e18.
    ...
    address[] public pathToBrew; // Path from staked token to BREW
    ...
    // Setting updates
    event SetPathToBrew(address[] oldPath, address[] newPath);
    ...
    constructor(
        ...
        require(
            _pathToBrew[0] == address(_farmRewardToken) && _pathToBrew[_pathToBrew.length - 1] == address(BREW),
            "CafeSingleVaults: Incorrect path to BREW"
        );

        require(
            _pathToWbnb[0] == address(_farmRewardToken) && _pathToWbnb[_pathToWbnb.length - 1] == WBNB,
            "CafeSingleVaults: Incorrect path to WBNB"
        );

        require(_buyBackRate <= buyBackRateUL);
        require(_platformFee <= platformFeeUL);

        AUTO_BREW = IBrewVault(_autoBrew);
     ...
     */
    modifier onlyKeeper() {
        require(keeper == msg.sender, "CafeSingleVaults: caller is not the keeper");
        _;
    }

    // 1. Harvest rewards
    // 2. Collect fees
    // 3. Convert rewards to $BREW
    // 4. Stake to brew auto-compound vault
    function earn(
        uint256 _minPlatformOutput,
        uint256 _minKeeperOutput,
        uint256 _minBurnOutput,
        uint256 _minBrewOutput
    ) external onlyKeeper {
        if (IS_CAKE_STAKING) {
        ...
        // Convert remaining rewards to BREW
        _swap(
            _rewardTokenBalance(),
            _minBrewOutput,
            pathToBrew,
        ...
        uint256 previousShares = totalAutoBrewShares();
        uint256 brewBalance = _brewBalance();
        ...
        AUTO_BREW.deposit(brewBalance);

        uint256 currentShares = totalAutoBrewShares();
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount > 0, "CafeSingleVaults: amount must be greater than zero");
        ...
        user.autoBrewShares = user.autoBrewShares.add(
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(_amount > 0, "CafeSingleVaults: amount must be greater than zero");
        require(user.stake >= _amount, "CafeSingleVaults: withdraw amount exceeds balance");
        ...
        // Withdraw brew rewards if user leaves
        if (user.stake == 0 && user.autoBrewShares > 0) {
            _claimRewards(user.autoBrewShares, false);
        }
    ...
    function _claimRewards(uint256 _shares, bool _update) private {
        ...
        require(user.autoBrewShares >= _shares, "CafeSingleVaults: claim amount exceeds balance");

        user.autoBrewShares = user.autoBrewShares.sub(_shares);

        uint256 brewBalanceBefore = _brewBalance();

        AUTO_BREW.withdraw(_shares);

        uint256 withdrawAmount = _brewBalance().sub(brewBalanceBefore);

        _safeBREWTransfer(msg.sender, withdrawAmount);
    ...
    function getExpectedOutputs() external view returns (
        ...
        uint256 brewOutputWithoutFees = _getExpectedOutput(pathToBrew);

        platformOutput = wbnbOutput.mul(platformFee).div(10000);
        keeperOutput = wbnbOutput.mul(keeperFee).div(10000);
        burnOutput = brewOutputWithoutFees.mul(buyBackRate).div(10000);

        brewOutput = brewOutputWithoutFees.sub(
            brewOutputWithoutFees.mul(platformFee).div(10000).add(
                brewOutputWithoutFees.mul(keeperFee).div(10000)
            ).add(
                brewOutputWithoutFees.mul(buyBackRate).div(10000)
            )
        );
    }
    ...
    function balanceOf(
        ...
        stake = user.stake;
        autoBrewShares = user.autoBrewShares.add(pendingShares);
        brew = autoBrewShares.mul(AUTO_BREW.getPricePerFullShare()).div(1e18);
    }
    ...
    function _brewBalance() private view returns (uint256) {
        return BREW.balanceOf(address(this));
    }
    ...
    function totalAutoBrewShares() public view returns (uint256) {
        (uint256 shares, , ,) = AUTO_BREW.userInfo(address(this));

        return shares;
    }

    // Safe BREW transfer function, just in case if rounding error causes pool to not have enough
    function _safeBREWTransfer(address _to, uint256 _amount) private {
        uint256 balance = _brewBalance();

        if (_amount > balance) {
            BREW.transfer(_to, balance);
        } else {
    ...
    function setPathToBrew(address[] memory _path) external onlyOwner {
        require(
            _path[0] == address(FARM_REWARD_TOKEN) && _path[_path.length - 1] == address(BREW),
            "CafeSingleVaults: Incorrect path to BREW"
        );

        address[] memory oldPath = pathToBrew;

        pathToBrew = _path;

        emit SetPathToBrew(oldPath, pathToBrew);
    }
    ...
    function setKeeperFee(uint256 _keeperFee) external onlyOwner {
        require(_keeperFee <= keeperFeeUL, "CafeSingleVaults: Keeper fee too high");
    ...
    function setPlatformFee(uint256 _platformFee) external onlyOwner {
        require(_platformFee <= platformFeeUL, "CafeSingleVaults: Platform fee too high");
    ...
    function setBuyBackRate(uint256 _buyBackRate) external onlyOwner {
        require(
            ...
            "CafeSingleVaults: Buy back rate too high"
        );
    ...
    function setEarlyWithdrawFee(uint256 _earlyWithdrawFee) external onlyOwner {
        require(
            ...
            "CafeSingleVaults: Early withdraw fee too high"
        );
